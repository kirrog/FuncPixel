...
def apply_limits_to_class(mnist_data_x, mnist_data_y, pictures_data, position, model):
    ...
    string = "Search: accuracy: {acc:.06f} position: {pos:06d} classes_acc: ".format(acc=res, pos=position)
...
model_save_path = "./data/models/cutted/{i:02d}.h5"

for i in range(len(pictures)):
    print("Class: {i:02d}".format(i=i))
    pictures[i].sort(key=lambda pict: pict.grad)
    apply_limits_to_class(mnist_test_x, mnist_test_y, pictures[i], positions[i], model)
    model.save(model_save_path.format(i=i))

    restore = []
    for i in save_model_weights:
